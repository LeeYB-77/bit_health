...
**진행 상태: Phase 1~7 완료 및 커밋. 테스트 254개 통과, 실 PostgreSQL 검증 완료. Phase 8(통합 검증 및 배포) 대기.**
...
- [x] 선착순 신청 — `booking_type='open'`, 즉시 `confirmed` (열린 항목 3 확정 후)
- [x] 확정 기간과 겹치면 거부
- [x] 선착순 확정 시에도 통보 + 추가입력 링크 발송
- [x] `main.py` `scheduled_jobs()` 확장 — 매월 1일 회차 자동 생성
- [x] 접수 마감 3일 전 관리자 리마인더
- [x] 마감일 경과 시 `status='closed'`
- [x] 통보일 자동 통보, 미확정 경합 남으면 보류 + 관리자 경고
- [x] ⚠️ 1분 주기 실행이므로 모든 작업에 중복 실행 방지 플래그 필수
- [x] ✅ 테스트: 선착순 즉시 확정, 겹침 거부
- [x] ✅ 테스트: 회차 중복 생성 방지
- [x] ✅ 테스트: 통보 보류 조건
- [x] 📦 커밋: `feat: 비트별장 선착순 예약 및 회차 자동화`
...
1. [x] 선착순 즉시 확정 — **신청 즉시 confirmed + 통보**로 구현 (Phase 7 완료)
2. [x] 성인+아동 합계 불일치 처리 — **경고만 표시하고 저장 허용**으로 구현 (Phase 6 완료)
3. [x] 통보일 미확정 건 처리 — **통보 보류 + 관리자 경고 1회**로 구현 (Phase 7 완료)
