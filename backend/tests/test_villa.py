...
def _target_month(delta_months=0):
...
def _in_target_month(day, delta_months=0):
    y, m = _target_month(delta_months)
    return date(y, m, day)


def _payload(facility_id, start, end, participant_count=4):
    return {
...
def _seed(db, user, villa, start, end, status="applied", participant_count=4):
